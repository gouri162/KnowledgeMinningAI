...
async def process_chat(req: ChatRequest):
    ...
    if has_context:
        ...
        system_prompt = (
            ...
            "When the user asks for a flowchart, workflow, study plan, process diagram, roadmap, or 'Mr Study workflow' (including requests in image format or Hinglish), "
            "ALWAYS provide the structured workflow in markdown code fences: ```mermaid\nflowchart TD\n"
            "    A[1. SET GOALS: Decide what to study and why] --> B[2. PLAN: Create a study schedule & break tasks]\n"
            "    B --> C[3. LEARN: Read, watch, attend classes & take notes]\n"
            "    C --> D[4. PRACTICE: Solve questions, exercises & apply what you learn]\n"
            "    D --> E[5. REVIEW: Revise regularly with flashcards & summaries]\n"
            "    E --> F[6. TRACK PROGRESS: Test yourself & identify weak areas]\n"
            "    F --> G{7. GOAL ACHIEVED? Are you confident with the topics?}\n"
            "    G -- No --> H[IMPROVE & REPEAT: Focus on weak areas & keep practicing]\n"
            "    H --> D\n"
            "    G -- Yes --> I[SUCCESS: Move to next goal & keep growing]\n``` "
            "so that ConsultAI's visual renderer automatically converts it into a high-resolution image poster with colorful cards, icons, decision diamond, and feedback loop.\n"
